...
#substitute and start_with
df %>%
  mutate(across(everything(.),~ gsub(",",".", .))) %>%
  mutate(across(starts_with("Importance"),~as.numeric(.x)))
...
design_matrix <- data.frame(A,W,Y)
  
design_matrix <- fastDummies::dummy_cols(design_matrix,
                        select_columns = "A",
                        )
...
#MSM with categorical A
{system.time(
  tmle_MSM_categA <- tmleMSM(Y=design_matrix$Y,
                     A=design_matrix$A,
                     W=select(design_matrix,c(W1,W2,W3)),
                     #T=design_matrix$year-1990,
                     #V=matrix(1, ncol=1, nrow=nrow(design_matrix)),#if no V
                     V=select(design_matrix,c(A_1,A_2,A_3)),#c(sex_num, age_num)
...
{system.time(
  tmle_MSM_contA <- tmleMSM(Y=design_matrix$Y,
                     A=design_matrix$A,
                     W=select(design_matrix,c(W1,W2,W3)),
                     #T=design_matrix$year-1990,
                     #V=matrix(1, ncol=1, nrow=nrow(design_matrix)),#if no V
                     V=select(design_matrix,V),#c(sex_num, age_num)
                     #id=design_matrix$location,#correlation within units
...
tmle_fit_from_spec <- tmle3(tmle_ATE(), cpp2, nodes, learner_list)#tmle_TSM_all
